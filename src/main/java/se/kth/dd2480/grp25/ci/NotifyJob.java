    ...
    try {
      ...
      try (OutputStream os = conn.getOutputStream()) {
          ...
          if (event.getStatus() == Event.Status.FAIL) {
            ...
            notifyEvent =
                new Event(
                    ...
                    "Notified that build failed.", event);
          } else {
          ...
          if (event.getStatus() == Event.Status.SUCCESSFUL) {
            ...
            notifyEvent =
                new Event(
                    ...
                    Event.Type.NOTIFY,
                    Event.Status.SUCCESSFUL,
                    "Notified that build was successful.", event);
          } else {
            ...
            notifyEvent =
                new Event(
                    ...
                    Event.Type.NOTIFY,
                    Event.Status.SUCCESSFUL,
                    "Notified that tests failed.", event);
          }
        }
        ...
        if (conn.getResponseCode() == 422) {
          queue.insert(
              new Event(
                  event.getId(), Event.Type.NOTIFY, Event.Status.FAIL, "Unprocessable entity", event));
        } else {
          queue.insert(notifyEvent);
        }
        os.flush();
